...
{
	private $url = "https://coinworxx.scavix.net/api";
	private $sandbox_url = "https://coinworxx.scavix.net/api";
	var $token = false;
	var $secret = false;
    var $response = false;
    var $success = false;
	...
	public function __construct($api_end_point, $token=false, $secret=false, $isSandbox = false)
	{
		$this->url = $api_end_point;
		$this->sandbox_url = $api_end_point;
		$this->token = $token;
		$this->secret = $secret;
	...
	protected function callApi($path,$arguments=array())
	{
        $this->response = $this->success = $this->result = $this->error = false;
         $url = $this->isSandbox?$this->sandbox_url:$this->url;
		...
		$ch = curl_init();
		CNovationPayGateway::log(__FILE__ . ' . ' . __LINE__ . ': '. $url);
